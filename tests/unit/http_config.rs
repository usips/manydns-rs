...
async fn test_local_address_binding_to_localhost_works() {
    ...
    assert!(result.is_ok(), "Provider creation with localhost binding should succeed");
}
...
async fn test_local_address_binding_to_unavailable_ip_fails_on_connect() {
    ...
    let config = HttpClientConfig::new()
        .local_address(IpAddr::V4(Ipv4Addr::new(192, 0, 2, 1)));
    ...
    assert!(provider.is_ok(), "Client creation should succeed even with unavailable IP");
}
